...
triangle = []
for row in triangle_text.split('\n'):
    if row.rstrip() != '':
        triangle.append(row.rstrip().split(' '))

# down and over, when doing the DOWN that's when the high determination is made
# addition of high values all the way down.  Should be able to back walk, if doing additions and taking the previous high that will either be UP or to the left.
